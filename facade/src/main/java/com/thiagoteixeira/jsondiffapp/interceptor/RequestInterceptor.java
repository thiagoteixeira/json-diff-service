...
public class RequestInterceptor implements HandlerInterceptor {
   ...
   *
   * When traceId has not been found into the current request then it generates one using {@link UUID#randomUUID()}
   *
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    ...
    MDC.put(ApiHeaders.TRACE_ID_HEADER, StringUtils.isBlank(value) ? UUID.randomUUID().toString(): value);
    return true;
  }
